...
class Game {
  ...
  constructor (options) {
    ...
    this.world.cameraMoveCounter = -1001;
  ...
  run () {
    ...
    for (let i = 0; i < 21; i++) {
      this.generateTrampolines(5, i * -100);
    }
  ...
  generateWorld () {
    if (this.world.cameraMoveCounter < -1000) {
      for (let i = 0; i < 11; i++) {
        this.generateTrampolines(4, this.assets.chicken.y - 1000 + (i * -100));
      }
